...
export async function POST(request: Request) {
    ...
    const [result] = await connection.query<ResultSetHeader>('INSERT INTO tasks (title, status) VALUES (?, ?)', [title, 'Todo']);
    await connection.end();
...
export async function PUT(request: Request) {
    ...
    await connection.query('UPDATE tasks SET title = ?, status = ? WHERE id = ?', [title, status, id]);
    await connection.end();
